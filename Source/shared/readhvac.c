...
void SetHVACInfo(hvacdatacollection *hvaccoll) {
  ...
  if(hvaccoll->hvacconnectinfo == NULL){
    NewMemory((void **)&hvaccoll->hvacconnectinfo, (hvaccoll->nhvacnodeinfo+hvaccoll->nhvacductinfo+1)*sizeof(hvacconnectdata));
    hvaccoll->nhvacconnectinfo = 0;
    for(i = 0;i < hvaccoll->nhvacductinfo;i++){
      hvacductdata *ducti;

      ducti = hvaccoll->hvacductinfo + i;
      if(HaveHVACConnect(ducti->connect_id, hvaccoll->hvacconnectinfo, hvaccoll->nhvacconnectinfo) == 1)continue;
      hvaccoll->hvacconnectinfo[hvaccoll->nhvacconnectinfo].index = ducti->connect_id;
    ...
    for(i = 0;i < hvaccoll->nhvacnodeinfo;i++){
      ...
      nodei = hvaccoll->hvacnodeinfo + i;
      if(HaveHVACConnect(nodei->connect_id, hvaccoll->hvacconnectinfo, hvaccoll->nhvacconnectinfo) == 1)continue;
      hvaccoll->hvacconnectinfo[hvaccoll->nhvacconnectinfo].index = nodei->connect_id;
      hvaccoll->hvacconnectinfo[hvaccoll->nhvacconnectinfo].display = 1;
      hvaccoll->nhvacconnectinfo++;
    }
    if(hvaccoll->nhvacconnectinfo > 0){
      ResizeMemory((void **)&hvaccoll->hvacconnectinfo, hvaccoll->nhvacconnectinfo * sizeof(hvacconnectdata));
      qsort((int *)hvaccoll->hvacconnectinfo, hvaccoll->nhvacconnectinfo, sizeof(hvacconnectdata), CompareHvacConnect);
      for(i = 0;i < hvaccoll->nhvacductinfo;i++){
        hvacductdata *ducti;
        int j;
...
void ReadHVACData0(hvacdatacollection *hvaccoll, int flag, FILE_SIZE *file_size) {
  ...
  *file_size   = GetFileSizeSMV(hvaccoll->hvacductvalsinfo->file);
  ...
  FREEMEMORY(hvaccoll->hvacductvalsinfo->times);
  NewMemory((void **)&hvaccoll->hvacductvalsinfo->times, nframes * sizeof(float));
  hvaccoll->hvacductvalsinfo->ntimes = nframes;

  FREEMEMORY(hvaccoll->hvacnodevalsinfo->times);
  NewMemory((void **)&hvaccoll->hvacnodevalsinfo->times, nframes * sizeof(float));
  ...
  for(i = 0;i < hvaccoll->hvacductvalsinfo->n_duct_vars;i++){
    ...
    hi = hvaccoll->hvacductvalsinfo->duct_vars + i;
    NewMemory((void **)&hi->vals, n_ducts*hvaccoll->hvac_maxcells*nframes * sizeof(float));
  }
  for(i = 0;i < hvaccoll->hvacnodevalsinfo->n_node_vars;i++){
    hvacvaldata *hi;
