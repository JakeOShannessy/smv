...
FILE_SIZE ReadIsoGeom(int ifile, int load_flag, int *geom_frame_index, int *errorcode){
  ...
  if(strcmp(isoi->surface_label.shortlabel,"hrrpuv")==0){
    surfi->color=GetColorPtr(global_scase.firstcolor, hrrpuv_iso_color);
  }
...
void ReadIsoOrig(const char *file, int ifile, int flag, int *errorcode){
  ...
  *errorcode = 0;
  ...
  if(strcmp(ib->surface_label.shortlabel,"FED")==0){
    ...
    fed_colors[0]=GetColorPtr(global_scase.firstcolor, fed_blue);
    fed_colors[1]=GetColorPtr(global_scase.firstcolor, fed_yellow);
    fed_colors[2]=GetColorPtr(global_scase.firstcolor, fed_red);
  }
  asurface=meshi->animatedsurfaces;
  break_frame=0;
  iitime=0;
