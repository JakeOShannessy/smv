...
extern "C" void GLUIBoundsSetup(int main_window){
  ...
  ROLLOUT_files = glui_bounds->add_rollout("Files", false, FILE_ROLLOUT);
  ...
  if(global_scase.npartinfo > 0 || global_scase.slicecoll.nsliceinfo > 0 || global_scase.slicecoll.nvsliceinfo > 0 || global_scase.nisoinfo > 0 || global_scase.npatchinfo || global_scase.smoke3dcoll.nsmoke3dinfo > 0 || global_scase.nplot3dinfo > 0){
    ROLLOUT_showhide = glui_bounds->add_rollout_to_panel(ROLLOUT_files,"Show/Hide", false, SHOWHIDE_ROLLOUT);
  ...
  if(smokezippath != NULL && (global_scase.npatchinfo > 0 || global_scase.smoke3dcoll.nsmoke3dinfo > 0 || global_scase.slicecoll.nsliceinfo > 0)){
    ROLLOUT_compress = glui_bounds->add_rollout_to_panel(ROLLOUT_files, "Compress", false, COMPRESS_ROLLOUT);
  ...
  ROLLOUT_script = glui_bounds->add_rollout_to_panel(ROLLOUT_files, "Scripts", false, SCRIPT_ROLLOUT);
  ...
  ROLLOUT_filebounds = glui_bounds->add_rollout("Data", false, DATA_ROLLOUT);
  TOGGLE_ROLLOUT(filedatacolprocinfo, nfiledatacolprocinfo, ROLLOUT_filebounds, DATA_ROLLOUT, glui_bounds);

  /*  zone (cfast) */

  if(global_scase.nzoneinfo > 0){
    ROLLOUT_zone_bound = glui_bounds->add_rollout_to_panel(ROLLOUT_filebounds, "Zone/slice temperatures", false, ZONE_ROLLOUT);
  ...
  if(global_scase.smoke3dcoll.nsmoke3dinfo > 0 || nvolrenderinfo > 0){
    ROLLOUT_smoke3d = glui_bounds->add_rollout_to_panel(ROLLOUT_filebounds, "3D smoke", false, SMOKE3D_ROLLOUT);
  ...
  if(global_scase.npatchinfo > 0){
    glui_active = 1;
    ROLLOUT_bound = glui_bounds->add_rollout_to_panel(ROLLOUT_filebounds, "Boundary", false, BOUNDARY_ROLLOUT);
    ...
    ROLLOUT_boundary_average = glui_bounds->add_rollout_to_panel(ROLLOUT_bound, "Average data", false, BOUNDARY_AVERAGE_ROLLOUT);
    ...
    ROLLOUT_boundary_settings = glui_bounds->add_rollout_to_panel(ROLLOUT_bound, "Settings", false, BOUNDARY_SETTINGS_ROLLOUT);
  ...
  if(global_scase.nplot3dinfo > 0){
    ...
    ROLLOUT_vector = glui_bounds->add_rollout_to_panel(PANEL_plot3d, "Vector", false, PLOT3D_VECTOR_ROLLOUT);
  ...
  if(global_scase.slicecoll.nsliceinfo > 0){
    ...
    ROLLOUT_slice_average = glui_bounds->add_rollout_to_panel(PANEL_slice_buttonsA, "Average data", false, SLICE_AVERAGE_ROLLOUT);
    ...
    ROLLOUT_line_contour = glui_bounds->add_rollout_to_panel(PANEL_slice_buttonsA, "Line contours", false, LINE_CONTOUR_ROLLOUT);
    ...
    if(nslicedups > 0){
      ROLLOUT_slicedups = glui_bounds->add_rollout_to_panel(ROLLOUT_slice, "Duplicates", false, SLICE_DUP_ROLLOUT);
  ...
  ROLLOUT_autoload = glui_bounds->add_rollout_to_panel(PANEL_loadbounds, "Auto load", false, LOAD_AUTO_ROLLOUT);
  ...
  ROLLOUT_time2 = glui_bounds->add_rollout_to_panel(PANEL_loadbounds, "Set time limits", false, LOAD_TIMEBOUND_ROLLOUT);
  ...
  ROLLOUT_memcheck = glui_bounds->add_rollout("Memory check",false,MEMCHECK_ROLLOUT);
