...
void Smoke3DShowMenu(int value){
  ...
  if(value<0){
    ...
    smoke3di = scase.smoke3dcoll.smoke3dinfo + value;
    if(plotstate!=DYNAMIC_PLOTS){
      plotstate=DYNAMIC_PLOTS;
      smoke3di->display=1;
    }
  ...
  case MENU_PLOT3D_HIDEALL:
   ...
   case DISPLAY_PLOT3D:
     for(i=0;i<scase.nplot3dinfo;i++){
       if(scase.plot3dinfo[i].loaded==1)scase.plot3dinfo[i].display=show_plot3dfiles;
     }
     break;
   default:
     if(value>=1000){
       if(plotstate==STATIC_PLOTS){
         scase.plot3dinfo[value-1000].display=1-scase.plot3dinfo[value-1000].display;
       }
       else{
         scase.plot3dinfo[value-1000].display=1;
       }
     }
     break;
  }
...
void LoadVolsmoke3DMenu(int value){
  ...
  if(value >= 0){
    ...
    meshi = scase.meshescoll.meshinfo + value;
...
void LoadUnloadMenu(int value){
  ...
  case RELOAD_INCREMENTAL_ALL:
    ...
    for(i = 0; i<scase.nplot3dinfo; i++){
      scase.plot3dinfo[i].finalize=0;
    }
    for(i = scase.nplot3dinfo-1; i>=0; i--){
      if(scase.plot3dinfo[i].loaded==1){
        scase.plot3dinfo[i].finalize = 1;
        break;
      }
    }
    int plot3d_loaded = 0;
    ...
    for(i=0;i<scase.nplot3dinfo;i++){
      if(scase.plot3dinfo[i].loaded==1){
        plot3d_loaded = 1;
        total_plot3d_filesize += ReadPlot3D(scase.plot3dinfo[i].file,i,LOAD,&errorcode);
        file_count++;
      }
    }
...
void UnLoadSmoke3DMenu(int value){
  ...
  if(value<0){
    ...
    UnloadSmoke3D(scase.smoke3dcoll.smoke3dinfo + value);
    SmokeWrapup();
  }
}
...
FILE_SIZE LoadSmoke3D(int type, int frame, int *count, float *time_value){
  ...
  if(load_only_when_unloaded == 0){
    for(i = scase.smoke3dcoll.nsmoke3dinfo - 1; i >= 0; i--){
      smoke3ddata *smoke3di;
...
#define MENU_SMOKE_16             -11
  ...
  if(value>=0){
    if(scriptoutstream!=NULL){
      ...
      file = scase.smoke3dcoll.smoke3dinfo[value].file;
      fprintf(scriptoutstream,"LOADFILE\n");
      fprintf(scriptoutstream," %s\n",file);
    }
    if(scriptoutstream==NULL||script_defer_loading==0){
    ...
    if(scase.smoke3dcoll.nsmoke3dtypes>0){
      ...
      PRINTF("3D smoke file sizes:\n");
      for(ii = 0; ii<scase.smoke3dcoll.nsmoke3dtypes; ii++){
        FILE_SIZE total_size;
        char *smoke_type;
    ...
    value = -(value + 100);
    smoke3di = scase.smoke3dcoll.smoke3dtypes[value].smoke3d;
    if(scriptoutstream!=NULL){
...
int AnySmoke(void){
  ...
  if(scase.smoke3dcoll.nsmoke3dinfo>0)return 1;
  return 0;
}
...
int LoadAllPlot3D(float time){
  ...
  for(i = 0; i < scase.nplot3dinfo; i++){
    ...
    total_plot3d_filesize += ReadPlot3D(plot3di->file, plot3di - scase.plot3dinfo, LOAD, &errorcode);
    file_count++;
...
void LoadPlot3dMenu(int value){
  ...
  if(value>=0){
    if(scriptoutstream!=NULL&&loadplot3dall==0){
      fprintf(scriptoutstream,"LOADPLOT3D\n");
      fprintf(scriptoutstream," %i %f\n",
        scase.plot3dinfo[value].blocknumber+1,scase.plot3dinfo[value].time);
    }
    if(scriptoutstream==NULL||script_defer_loading==0){
      for(i = 0;i < scase.nplot3dinfo;i++){
        plot3ddata *plot3di;
      ...
      scase.plot3dinfo[value].loadnow = 1;
      SetLoadedPlot3DBounds();
      scase.plot3dinfo[value].finalize = 1;
      for(i = 0; i < scase.nplot3dinfo; i++){
        plot3ddata *plot3di;
      ...
      for(i = 0;i < 1;i++){
        ...
        plot3di = scase.plot3dinfo + value;
        IF_NOT_USEMESH_CONTINUE(plot3di->loaded, plot3di->blocknumber);
    ...
    for(i = 0; i<scase.nplot3dinfo; i++){
      ...
      plot3di = scase.plot3dinfo + i;
      if(plot3di->loaded==1){
        ReadPlot3D("", plot3di-scase.plot3dinfo, UNLOAD, &errorcode);
      }
    }
    FILE_SIZE total_plot3d_filesize = 0;
    int file_count=0;
...
void BlockageMenu(int value){
  ...
  switch(value){
   ...
   case BLOCKlocation_exact:
   case BLOCKlocation_cad:
     if(NCADGeom(&scase.cadgeomcoll) == 0){
...
void ShowObjectsMenu(int value){
  ...
  if(value>=0&&value<scase.objectscoll.nobject_defs){
    objecti = scase.objectscoll.object_defs[value];
...
/* --------------------------------static variable menu -------------------------- */
  ...
  if(scase.nplot3dinfo>0){
    ...
    for(n=0;n<numplot3dvars;n++){
      ...
      p3label = scase.plot3dinfo[0].label[n].shortlabel;
      if(plotn-1==n){
        char menulabel[1024];
...
/* --------------------------------Plot3d Show menu -------------------------- */
  if(scase.nplot3dinfo>0){
    ...
    if(nplot3dloaded>0){
      ...
      for(ii = 0;ii<scase.nplot3dinfo;ii++){
...
/* --------------------------------terrain_obst_showmenu -------------------------- */
  ...
  if(scase.objectscoll.nobject_defs>0||hrrptr!=NULL){
  ...
  if(scase.objectscoll.nobject_defs>0){
    ...
    if(scase.objectscoll.nobject_defs>0&&scase.devicecoll.ndeviceinfo>0){
      glutAddMenuEntry("-",MENU_DUMMY);
...
/* --------------------------------label menu -------------------------- */
  ...
  if(LabelGetNUserLabels(&scase.labelscoll) > 0){
...
/* --------------------------------iso show menu -------------------------- */
    ...
    if(scase.nisoinfo>0&&ReadIsoFile==1){
      ...
      if((visAIso & 4) != 4)glutAddMenuEntry(_("Points"), MENU_ISOSHOW_POINTS);
      hmesh=scase.meshescoll.meshinfo+highlight_mesh;
...
/* --------------------------------showslice menu -------------------------- */
  ...
  if(scase.tourcoll.ntourinfo>0){
    ...
    for(i=0;i<scase.tourcoll.ntourinfo;i++){
      ...
      if(glui_avatar_index_local>=0&&glui_avatar_index_local<scase.objectscoll.navatar_types){
        sv_object *avatari;

        avatari=scase.objectscoll.avatar_types[glui_avatar_index_local];
        strcat(menulabel,"(");
...
/* --------------------------------unload and load 3d vol smoke menus -------------------------- */
    ...
    /* --------------------------------unload and load 3d smoke menus -------------------------- */
      ...
      if(nsmoke3dloaded>0){
        ...
        for(i = 0; i<scase.smoke3dcoll.nsmoke3dtypes; i++){
          ...
          is_zlib = 0;
          for(j = 0; j<scase.smoke3dcoll.nsmoke3dinfo; j++){
            if(scase.smoke3dcoll.smoke3dinfo[j].loaded==1&&scase.smoke3dcoll.smoke3dinfo[j].type==i){
              if(scase.smoke3dcoll.smoke3dinfo[j].compression_type == COMPRESSED_ZLIB){
                is_zlib = 1;
              }
              doit = 1;
              break;
          ...
          if(doit == 1){
            ...
            strcpy(smvmenulabel, scase.smoke3dcoll.smoke3dtypes[i].longlabel);
            if(is_zlib == 1){
      ...
      }
    {
      if(scase.smoke3dcoll.nsmoke3dinfo>0){
        ...
        for(ii = 0; ii<scase.smoke3dcoll.nsmoke3dtypes; ii++){
          if(scase.meshescoll.nmeshes>1){
            CREATEMENU(scase.smoke3dcoll.smoke3dtypes[ii].menu_id, LoadSmoke3DMenu);
          }
          for(i = 0; i<scase.smoke3dcoll.nsmoke3dinfo; i++){
            char menulabel[256];
            smoke3ddata *smoke3di;
        ...
        if(scase.meshescoll.nmeshes>1){
          ...
          for(ii = 0; ii<scase.smoke3dcoll.nsmoke3dtypes; ii++){
            ...
            nloaded=0;
            is_zlib = 0;
            for(jj=0;jj<scase.smoke3dcoll.nsmoke3dinfo;jj++){
              if(scase.smoke3dcoll.smoke3dinfo[jj].type==ii){
                if(scase.smoke3dcoll.smoke3dinfo[jj].loaded==1)nloaded++;
                if(scase.smoke3dcoll.smoke3dinfo[jj].compression_type == COMPRESSED_ZLIB){
                  is_zlib = 1;
                }
                ntotal++;
              }
            ...
            strcat(menulabel, scase.smoke3dcoll.smoke3dtypes[ii].longlabel);
            strcat(menulabel, scase.smoke3dcoll.smoke3dtypes[ii].smoke3d->cextinct);
            if(is_zlib == 1){
              strcat(menulabel, "(ZLIB)");
            }
    ...
    if(scase.nplot3dinfo>0){
      ...
      for(ii=1;ii<scase.nplot3dinfo;ii++){
        int im1;

        i = plot3dorderindex[ii];
        im1 = plot3dorderindex[ii-1];
        plot3di = scase.plot3dinfo + i;
        plot3dim1 = scase.plot3dinfo + im1;
...
/* --------------------------------loadunload menu -------------------------- */
 ...
 //       GLUTADDSUBMENU(_("Terrain"),loadterrainmenu);
      ...
      if(scase.nplot3dinfo>0)GLUTADDSUBMENU(_("Plot3D"),loadplot3dmenu);
