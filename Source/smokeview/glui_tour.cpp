...
extern "C" void GLUITourSetup(int main_window){
  ...
  glui_tour->add_spinner_to_panel(PANEL_path, _("start time"), GLUI_SPINNER_FLOAT, &scase.tourcoll.tour_tstart, VIEW_times, TourCB);
  glui_tour->add_spinner_to_panel(PANEL_path, _("stop time:"), GLUI_SPINNER_FLOAT, &scase.tourcoll.tour_tstop, VIEW_times, TourCB);
  glui_tour->add_spinner_to_panel(PANEL_path, _("points"),     GLUI_SPINNER_INT,   &scase.tourcoll.tour_ntimes, VIEW_times, TourCB);
  ...
  if(scase.objectscoll.navatar_types > 0){
    ...
    for(i = 0;i < scase.objectscoll.navatar_types;i++){
      LISTBOX_avatar->add_item(i, scase.objectscoll.avatar_types[i]->label);
    }
    if(tourlocus_type == 0){
      glui_avatar_index = -1;
    }
      ...
      glui_avatar_index = scase.objectscoll.iavatar_types;
    }
...
int NextTour(void){
  ...
  if(i > scase.tourcoll.ntourinfo - 1)i = 0;
  if(i >= 0 && i < scase.tourcoll.ntourinfo){
    selectedtour_index = i;
    selected_tour = scase.tourcoll.tourinfo + i;
...
int PrevTour(void){
  ...
  i = selectedtour_index - 1;
  if(i < 0)i = scase.tourcoll.ntourinfo - 1;
  if(i >= 0 && i < scase.tourcoll.ntourinfo){
    selectedtour_index = i;
    selected_tour = scase.tourcoll.tourinfo + i;
    selected_frame = selected_tour->first_frame.next;
...
void TourCB(int var){
  ...
  if(scase.tourcoll.ntourinfo==0&&var!=TOUR_INSERT_NEW&&var!=TOUR_INSERT_COPY&&var!=TOUR_CLOSE&&var!=SAVE_SETTINGS_TOUR){
    return;
  }
  ...
  case TOUR_CIRCULAR_UPDATE:
    ...
    DeleteTourFrames(scase.tourcoll.tourinfo);
    InitCircularTour(scase.tourcoll.tourinfo,ncircletournodes,UPDATE);
  ...
  case SAVE_SETTINGS_TOUR:
    ...
    break;
  case SHOWTOURROUTE:
    if(edittour==1&&selectedtour_index<0&&scase.tourcoll.ntourinfo>0){
      selectedtour_index=0;
      TourCB(TOUR_LIST);
    }
    edittour = 1 - edittour;
  ...
  case KEYFRAME_viewXYZ:
    if(selected_frame!=NULL){
      if(selected_tour-scase.tourcoll.tourinfo==0)dirtycircletour=1;
  ...
  case KEYFRAME_NEXT:
    show_tour_hint = 0;
    if(selected_frame==NULL&&scase.tourcoll.tourinfo!=NULL){
      selected_frame=&(scase.tourcoll.tourinfo[0].first_frame);
      selected_tour=scase.tourcoll.tourinfo;
    }
    if(selected_frame!=NULL){
      thistour=selected_tour;
  ...
  case KEYFRAME_PREVIOUS:
    show_tour_hint = 0;
    if(selected_frame==NULL&&scase.tourcoll.tourinfo!=NULL){
      selected_frame=&(scase.tourcoll.tourinfo[0].last_frame);
      selected_tour=scase.tourcoll.tourinfo;
    }
    if(selected_frame!=NULL){
      thistour=selected_tour;
  ...
  case KEYFRAME_DELETE:
    ...
    if(selected_frame!=NULL){
      ...
      if(selected_frame!=NULL){
        ...
        if(thistour!=NULL)DeleteTour(thistour-scase.tourcoll.tourinfo);
      }
    }
    break;
  case TOUR_AVATAR:
...
// hack to make touring avatar show up
      scase.objectscoll.avatar_types[glui_avatar_index]->visible=1;
      updatemenu=1;
    }
    if(glui_avatar_index==-1){
      ...
      tourlocus_type=2;
      scase.objectscoll.iavatar_types=glui_avatar_index;
    }
    break;
  ...
  case TOUR_REVERSE:
    if(selectedtour_index>=0&&selectedtour_index<scase.tourcoll.ntourinfo){
      ReverseTour(scase.tourcoll.tourinfo[selectedtour_index].label);
    }
    break;
  ...
  case TOUR_INSERT_COPY:
    if(var==TOUR_INSERT_NEW){
      ...
      if(selectedtour_index>=0&&selectedtour_index<scase.tourcoll.ntourinfo){
        ...
        strcpy(label, scase.tourcoll.tourinfo[selectedtour_index].label);
    ...
    selected_tour=thistour;
    selectedtour_index = thistour - scase.tourcoll.tourinfo;
    selectedtour_index_old=selectedtour_index;
    GLUISetTourKeyframe();
    CreateTourPaths();
    UpdateViewTour();
  ...
  case TOUR_LABEL:
    if(thistour!=NULL){
      ...
      if(LISTBOX_tour!=NULL){
        LISTBOX_tour->delete_item(thistour-scase.tourcoll.tourinfo);
        LISTBOX_tour->add_item(thistour-scase.tourcoll.tourinfo,thistour->label);
      }
      UpdateTourMenuLabels();
      updatemenu=1;
    }
  ...
  case TOUR_HIDE:
    if(thistour!=NULL){
      if(tour_hide==1){
        thistour->display=1;
        TOURMENU(thistour-scase.tourcoll.tourinfo);
        NextTour();
        GLUISetTourKeyframe();
        thistour->display=0;
      }
...
extern "C" void GLUICreateTourList(void){
  ...
  if(selectedtour_index>=-1&&selectedtour_index<scase.tourcoll.ntourinfo)LISTBOX_tour->set_int_val(selectedtour_index);
...
extern "C" void GLUIUpdateTourControls(void){
  ...
  if(selected_tour!=NULL){
    selectedtour_index = selected_tour-scase.tourcoll.tourinfo;
    LISTBOX_tour->set_int_val(selectedtour_index);
  }
  else{
    selectedtour_index = TOURINDEX_MANUAL;
