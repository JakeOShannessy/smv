...
static void connection_cb(struct ev_loop *loop, ev_io *w, int revents) {
  ...
  if (!bytes_read) {
    ...
    if (server->debug_level) printf("Client closed connection.\n");
    return close_connection(loop, w);
  }
  else {
    cJSON *root;
    ...
    if ((root = cJSON_ParseWithOpts(conn->buffer, &end_ptr, cJSON_False)) !=
        NULL) {
...
static void accept_cb(struct ev_loop *loop, ev_io *w, int revents) {
  ...
  connection_watcher->fd =
      accept(w->fd, (struct sockaddr *)&their_addr, &sin_size);
  if (connection_watcher->fd == -1) {
    ...
    // copy debug_level, struct jrpc_connection has no pointer to struct
    // jrpc_server
...
static int __jrpc_server_start(struct jrpc_server *server) {
  ...
  if (server->debug_level) printf("server: waiting for connections...\n");
...
void jrpc_server_run(struct jrpc_server *server) { EV_RUN(server->loop, 0); }
