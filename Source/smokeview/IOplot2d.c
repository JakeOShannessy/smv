...
void SetupPlot2DUnitData(void){
  ...
  if(global_scase.nhrrinfo > 0){
    ...
    nhrrunits = 0;
    FREEMEMORY(hrrunits);
    NewMemory((void **)&hrrunits, global_scase.nhrrinfo * sizeof(hrrdata *));
    for(i = 0; i < global_scase.nhrrinfo; i++){
      int j;
      hrrdata *hrri;
      int skip_hrr;

      hrri = global_scase.hrrinfo + i;
...
void InitPlot2D(plot2ddata *plot2di, int plot_index){
  if(global_scase.devicecoll.ndeviceinfo == 0 && global_scase.nhrrinfo == 0)return;
  plot2di->ncurves = 0;
