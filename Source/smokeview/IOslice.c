...
float Get3DSliceVal(slicedata *sd, float *xyz){
  ...
  valmesh = meshescoll.meshinfo + sd->blocknumber;
...
void UpdateSliceMenuLabels(sliceparmdata *sp){
  ...
  if(sp->nsliceinfo>0){
    ...
    sd = slicecoll.sliceinfo + sextras.sliceorderindex[0];
    ...
    if(meshescoll.nmeshes>1){
      ...
      slicemesh = meshescoll.meshinfo + sd->blocknumber;
...
void UpdateVectorSkipNonUniform(slicedata *slicei, int factor_x, int factor_y, int factor_z){
  ...
  slicemesh = meshescoll.meshinfo + slicei->blocknumber;
...
void UpdateVsliceMenuLabels(sliceparmdata *sp){
  ...
  if(sp->nvsliceinfo>0){
    ...
    vsd = slicecoll.vsliceinfo + sextras.vsliceorderindex[0];
    ...
    for(i=1;i<sp->nvsliceinfo;i++){
      vsdold = slicecoll.vsliceinfo + sextras.vsliceorderindex[i - 1];
      sdold = slicecoll.sliceinfo + vsdold->ival;
      vsd = slicecoll.vsliceinfo + sextras.vsliceorderindex[i];
      sd = slicecoll.sliceinfo + vsd->ival;
...
int IsSliceDuplicate(multislicedata *mslicei, int ii, int flag){
  ...
  for(jj=0;jj<mslicei->nslices;jj++){ // identify duplicate slices
    ...
    meshj = meshescoll.meshinfo+slicej->blocknumber;
...
void UpdateSliceinfoPtrs(sliceparmdata *sp){
  ...
  meshescoll.meshinfo->isliceinfo    = 0;
  for(i=1; i<meshescoll.nmeshes; i++){
    meshdata *meshim1, *meshi;

    meshim1               = meshescoll.meshinfo + i - 1;
    meshi                 = meshescoll.meshinfo + i;
...
void GetSliceParams(sliceparmdata *sp){
  ...
  if(sp->nsliceinfo>0){
    ...
    NewMemory((void **)&sextras.sliceorderindex,sizeof(int)*sp->nsliceinfo);
    for(i=0;i<sp->nsliceinfo;i++){
      sextras.sliceorderindex[i]=i;
    }
    qsort( (int *)sextras.sliceorderindex, (size_t)sp->nsliceinfo, sizeof(int), SliceCompare );
...

    {
      ...
      mslicei->nslices=1;
      sd = slicecoll.sliceinfo + sextras.sliceorderindex[0];
      mslicei->islices[0] = sextras.sliceorderindex[0];
      for(i=1;i<sp->nsliceinfo;i++){
        slicedata *sdold;
        ...
        mslicei->nslices++;
        mslicei->islices[mslicei->nslices-1]=sextras.sliceorderindex[i];
      }
    }
    have_multislice = 0;
...
void *UpdateVSlices(void *arg){
  ...
  max_dx = meshescoll.meshinfo->xplt_orig[1] - meshescoll.meshinfo->xplt_orig[0];
  max_dy = meshescoll.meshinfo->yplt_orig[1] - meshescoll.meshinfo->yplt_orig[0];
  max_dz = meshescoll.meshinfo->zplt_orig[1] - meshescoll.meshinfo->zplt_orig[0];
  for(i = 1; i<meshescoll.nmeshes; i++){
    meshdata *meshi;
    float *xplt, *yplt, *zplt;
  ...
  for(i=0;i<sp->nsliceinfo;i++){ //slow
    ...
    sdi = slicecoll.sliceinfo+i;
    meshi = meshescoll.meshinfo + sdi->blocknumber;
    vd->iu=-1;
    vd->iv=-1;
    vd->iw=-1;
    vd->ival=i;
  ...
  if(sp->nvsliceinfo>0){
    ...
    NewMemory((void **)&sextras.vsliceorderindex,sizeof(int)*sp->nvsliceinfo);
    for(i=0;i<sp->nvsliceinfo;i++){
      sextras.vsliceorderindex[i]=i;
    }
    qsort( (int *)sextras.vsliceorderindex, (size_t)sp->nvsliceinfo, sizeof(int), VSliceCompare );
    ...
    mvslicei->nvslices=1;
    vsd = slicecoll.vsliceinfo + sextras.vsliceorderindex[0];
    mvslicei->ivslices[0] = sextras.vsliceorderindex[0];
    ...
    for(i=1;i<sp->nvsliceinfo;i++){
      ...
      mvslicei->nvslices++;
      mvslicei->ivslices[mvslicei->nvslices-1]=sextras.vsliceorderindex[i];
    }
...
// load only portion of slice file written to since last time it was loaded (load_flag=RELOAD)
    ...
    xplt_local = meshescoll.meshinfo[blocknumber].xplt;
    yplt_local = meshescoll.meshinfo[blocknumber].yplt;
    zplt_local = meshescoll.meshinfo[blocknumber].zplt;
...
#define BOUND_PERCENTILE_DRAW          120
    ...
    if(use_set_slicecolor==0||set_slicecolor==SET_SLICECOLOR){
      if(sd->compression_type==UNCOMPRESSED){
        ...
        UpdateAllSliceLabels(slicefile_labelindex, errorcode);
        MakeColorLabels(sb->colorlabels, sb->colorvalues, qmin, qmax, sextras.nrgb);
      }
    }
    if(sd->compression_type == COMPRESSED_ZLIB){
      int ii;
  ...
  if(sd->volslice == 1){
    ...
    meshj = meshescoll.meshinfo + sd->blocknumber;
...
void DrawVolSliceTerrain(const slicedata *sd){
  ...
  if((sd->volslice == 1 && plotz >= 0 && visz_all == 1) || (sd->volslice == 0 && sd->idir == ZDIR)){
    ...
    glTranslatef(-sextras.xbar0,-sextras.ybar0,-sextras.zbar0 + voffset);
    ...
    for(i = sd->is1; i<maxi; i+=slice_skip){
      ...
      for(j = sd->js1; j<sd->js2; j += slice_skip){
        ...
        if(z11<sextras.zbar0||z31<sextras.zbar0||z33<sextras.zbar0)draw123=0;
        if(z11<sextras.zbar0||z33<sextras.zbar0||z13<sextras.zbar0)draw134=0;
    ...
    for(icol2 = 1;icol2<nx+ny-2;icol2++){
      ...
      for(i = iimin; i<nx-1; i++){
        ...
        for(k = sd->ks1; k<sd->ks2; k++){
          ...
          n++; n2++;ijk+=nxy;
          if(sextras.show_slice_in_obst==ONLY_IN_SOLID && c_iblank_x != NULL&&c_iblank_x[ijk]==GASGAS)continue;
          if(sextras.show_slice_in_obst==ONLY_IN_GAS   && c_iblank_x != NULL&&c_iblank_x[ijk]!=GASGAS)continue;
  ...
  if(visy_all==1){
    ...
    for(jrow2 = 1;jrow2<nx+ny-2;jrow2++){
      ...
      for(i = iimin; i<nx-1; i++){
        ...
        for(k = sd->ks1; k<sd->ks2; k++){
          n++; n2++; ijk+=nxy;
          if(sextras.show_slice_in_obst==ONLY_IN_SOLID && c_iblank_y!=NULL&&c_iblank_y[ijk]==GASGAS)continue;
          if(sextras.show_slice_in_obst==ONLY_IN_GAS   && c_iblank_y!=NULL&&c_iblank_y[ijk]!=GASGAS)continue;
          if(skip_slice_in_embedded_mesh==1&&iblank_embed!=NULL&&iblank_embed[ijk]==EMBED_YES)continue;
          r11 = (float)sd->iqsliceframe[n] / 255.0;
          r31 = (float)sd->iqsliceframe[n2] / 255.0;
          r13 = (float)sd->iqsliceframe[n + 1] / 255.0;
