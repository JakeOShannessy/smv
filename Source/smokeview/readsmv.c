...
int GetHrrCsvCol(char *label){
  ...
  if(label==NULL||strlen(label)==0||global_scase.nhrrinfo==0)return -1;
...
// construct column for each MLR column by heat of combustion except for air and products
  for(i = global_scase.nhrrinfo-nhrrhcinfo; i<global_scase.nhrrinfo; i++){
    hrrdata *hi;
    ...
    if(hi->base_col>=0){
      ...
      hi_from = global_scase.hrrinfo+hi->base_col;
...
void ReadHRR(int flag){
  ...
  if(global_scase.nhrrinfo>0){
    ...
    FREEMEMORY(global_scase.hrrinfo);
    global_scase.nhrrinfo = 0;
  }
  time_col  = -1;
  hrr_col   = -1;
  qradi_col = -1;
  ...
  global_scase.nhrrinfo = ncols;

  if(global_scase.nhrrinfo == 0)return;
  // allocate memory
  ...
  NewMemory((void **)&labels,                  global_scase.nhrrinfo*sizeof(char *));
  NewMemory((void **)&units,                   global_scase.nhrrinfo*sizeof(char *));
  NewMemory((void **)&global_scase.hrrinfo,  2*global_scase.nhrrinfo*sizeof(hrrdata));
NewMemory((void **)&vals,                      global_scase.nhrrinfo*sizeof(float));
NewMemory((void **)&valids,                    global_scase.nhrrinfo*sizeof(int));

// initialize each column
  for(i = 0; i<2*global_scase.nhrrinfo; i++){
    hrrdata *hi;
...
// find column index of several quantities
  ...
  if(time_col>=0)timeptr = global_scase.hrrinfo+time_col;

  hrr_col   = GetHrrCsvCol("HRR");
  if(hrr_col>=0&&time_col>=0)hrrptr = global_scase.hrrinfo+hrr_col;
...
// define column for each MLR column by heat of combustion except for air and products
  ...
  for(i = 0; i<global_scase.nhrrinfo; i++){
    ...
    if(strlen(hi->label.longlabel)>3&&strncmp(hi->label.longlabel,"MLR_",4)==0){
      ...
      if(doit==0)continue;
      hi2 = global_scase.hrrinfo + global_scase.nhrrinfo + nhrrhcinfo;
      hi2->base_col = i;
      strcpy(label, "HOC*");
      ...
      mlr_col = hi2-global_scase.hrrinfo;
      have_mlr = 1;
      nhrrhcinfo++;
    }
  }
...
//define column of hrr/qradi
  if(hrr_col>=0&&qradi_col>=0){
    ...
    chirad_col = global_scase.nhrrinfo+nhrrhcinfo;
    hi_chirad = global_scase.hrrinfo+chirad_col;
  ...
  global_scase.nhrrinfo += nhrrhcinfo;
// construct column for each MLR column by heat of combustion except for air and products
  for(i = global_scase.nhrrinfo- nhrrhcinfo; i<global_scase.nhrrinfo; i++){
    hrrdata *hi;
...
//construct column of qradi/hrr
  if(hrr_col>=0&&qradi_col>=0){
    ...
    hi_chirad = global_scase.hrrinfo+chirad_col;
    hi_hrr = global_scase.hrrinfo+hrr_col;
    hi_qradi = global_scase.hrrinfo+qradi_col;
...
// free arrays that were not used
  for(i = global_scase.nhrrinfo; i<2*(global_scase.nhrrinfo-nhrrhcinfo); i++){
    hrrdata *hi;
  ...
  for(;;){
    ...
    if(Match(buffer, "OBST")==1){
      ...
      for(i = 0; i<scase->obstcoll.nobstinfo; i++){
        ...
        if(colorindex>=0){
          obi->color = GetColorPtr(scase->rgb[scase->nrgb+colorindex]);
          obi->usecolorindex=1;
    ...
    if(MatchSMV(buffer,"THCP") == 1){
      ...
      if(meshi->ntc>0){
        ...
        for(nn=0;nn<meshi->ntc;nn++){
          ...
          InitDevice(&global_scase, devicecopy,xyz,0,NULL,NULL,xyznorm,0,0,NULL,"target");
    ...
    if(MatchSMV(buffer,"SPRK") == 1){
      ...
      if(meshi->nspr>0){
        ...
        for(nn=0;nn<meshi->nspr;nn++){
          ...
          InitDevice(&global_scase, devicecopy,NULL,0,NULL,NULL,xyznorm,0,0,NULL,NULL);
  ...
  */
    if(MatchSMV(buffer,"SMOD") == 1){
      ...
      for(nn=0;nn<sdnum;nn++){
        ...
        InitDevice(&global_scase, devicecopy,xyz,0,NULL,NULL,xyznorm,0,0,NULL,NULL);
  ...
  */
    if(MatchSMV(buffer,"OBST") == 1){
      ...
      for(iblock=0;iblock<n_blocks;iblock++){
        ...
        InitObst(&global_scase, bc,surfacedefault,nn+1,iobst-1);
        FGETS(buffer,255,stream);
        ...
        SetSurfaceIndex(&global_scase, bc);
      }
