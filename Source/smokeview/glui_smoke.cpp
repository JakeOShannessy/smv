...
extern "C" void GLUI3dSmokeSetup(int main_window){
  ...
  ROLLOUT_smokecolor = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_smoke3d, "Smoke color",false, SMOKECOLOR_ROLLOUT);
  ...
  if(colorbars.ncolorbars > 0){
    ROLLOUT_firecolor = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_smoke3d, "Fire color", false, FIRECOLOR_ROLLOUT);
  ...
  ROLLOUT_skip = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_smoke3d, "Planes", false, SKIP_ROLLOUT);
  ...
  if(nvolrenderinfo > 0){
    ROLLOUT_volume = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_smoke3d, "Volume render settings", false, VOLRENDER_ROLLOUT);
    TOGGLE_ROLLOUT(smokeprocinfo, nsmokeprocinfo, ROLLOUT_volume, VOLRENDER_ROLLOUT, glui_3dsmoke);

    //*** display

    ROLLOUT_voldisplay = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_volume,"Display",false, VOLSMOKE_DISPLAY);
    ...
    ROLLOUT_volsmoke_move = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_volume, "Scene movement", false, VOLSMOKE_MOVEMENT);
    ...
    ROLLOUT_volsmoke_compute = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_volume, "Compute", false, VOLSMOKE_COMPUTE_ROLLOUT);
    ...
    ROLLOUT_volsmoke_load = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_volume, "Load", false, VOLSMOKE_LOAD_ROLLOUT);
    ...
    ROLLOUT_generate_images = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_volume, "Generate images", false, VOLSMOKE_IMAGES_ROLLOUT);
    ...
    ROLLOUT_loadframe = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_volume, "Load frame", false, VOLSMOKE_LOADFRAME_ROLLOUT);
