...
void MouseEditColorbar(int button, int state, int x, int y) {
  ...
  if (val > 0 && val <= cbi->nnodes) {
    ...
    /* need to start colors at 1 so that black (color 0,0,0) is not interpreted
     * as a blockage */
...
void MouseEditTour(int button, int state, int x, int y) {
  ...
  val = val1;
  if (val != 0 && itourknots >= 0 && itourknots < ntourknots &&
...
void MouseEditBlockage(int button, int state, int x, int y) {
  ...
  if (val > 0 && val <= ntotalfaces) {
    ...
    for (i = 0; i < 6; i++) {
      ...
      surface_indices_bak[i] =
          inv_sorted_surfidlist[bchighlight->surf_index[i]];
    }
  ...
  if (val > 0 && val < ndeviceinfo) {
    ...
    if (devicei->labelptr != NULL && strcmp(devicei->labelptr, "null") != 0) {
      PRINTF("Selected Device: index=%i location:(%f,%f,%f) label:%s\n", val,
             xyz[0], xyz[1], xyz[2], devicei->labelptr);
    } else {
...
void CheckTimeBound(void) {
  ...
  if ((timebar_drag == 0 && itimes > nglobal_times - 1) ||
      (timebar_drag == 1 && itimes < 0)) {
    if (timebar_drag == 0) {
  ...
  if ((timebar_drag == 0 && itimes < 0) ||
      (timebar_drag == 1 && itimes > nglobal_times - 1)) {
    izone = nzone_times - 1;
...
int GetColorbarIndex(int flag, int x, int y) {
  ...
  if (flag == 0 || (vcolorbar_left_pos <= x && x <= vcolorbar_right_pos)) {
    ...
    if (vcolorbar_down_pos <= y && y <= vcolorbar_top_pos) {
      int index;
      index = CLAMP(255 * (float)(y - vcolorbar_down_pos) /
                        (float)(vcolorbar_top_pos - vcolorbar_down_pos),
                    0, 255);
      return index;
    } else {
...
int GetTimeFrame(int xm) {
  ...
  if (global_times != NULL && timebar_right_pos > timebar_left_pos) {
    ...
    factor = (float)(xm - timebar_left_pos) /
             (float)(timebar_right_pos - timebar_left_pos);
    factor = CLAMP(factor, 0.0, 1.0);
    time = global_times[0] * (1.0 - factor) +
           global_times[nglobal_times - 1] * factor;
...
int TimebarClick(int xm, int ym) {
  if (screenHeight - ym < titlesafe_offset + VP_timebar.height &&
      nglobal_times > 0) {
    //    PRINTF("ngt=%i xl=%i x=%i
...
void UpdateMouseInfo(int flag, int xm, int ym) {
  ...
  if (mi->direction[0] == 0 && mi->direction[1] == 0) {
    ...
    sum = mi->direction[0] * mi->direction[0] +
          mi->direction[1] * mi->direction[1];
    sum = sqrt(sum);
  ...
  if (mi->region == 0) {
    ...
    if (delta_angle != 0.0 &&
        ABS(mi->xdirection[0]) + ABS(mi->xdirection[1]) > 0.0) {
      float axis[3];
      ...
      if (key_state == KEY_SHIFT) {
        ...
        if ((inside == 1 && dy < 0.0) || (inside == 0 && dx < 0.0))
...
#ifdef _DEBUG
  {
    ...
    if (denom > 0.0) {
      PRINTF("angle= %f axis=(%f,%f,%f)\n",
             2.0 * RAD2DEG * acos(quat_general[0]), quat_general[1] / denom,
             quat_general[2] / denom, quat_general[3] / denom);
    } else {
...
void MouseButtonCB(GLFWwindow *window, int button, int action, int mods) {
  ...
  if (vis_gslice_data == 1 || show_gslice_triangles == 1 ||
      show_gslice_triangulation == 1) {
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT) {
      if (blockageSelect == 1) {
        ...
        if (structured_isopen == 1 && unstructured_isopen == 0)
          MouseEditBlockage(button, action, xpos, ypos);
      }
      if (edittour == 1 && blockageSelect == 0)
        MouseEditTour(button, action, xpos, ypos);
      ...
      if (select_geom != GEOM_PROP_NONE)
        MouseSelectGeom(button, action, xpos, ypos);
    }
    GLUTPOSTREDISPLAY;
...
void DragColorbarEditNode(int xm, int ym) {
  ...
  case KEY_CTRL: {
    ...
    if (key_state != KEY_ALT)
      xy[0] = (float)(xm - mouse_down_xy0[0]) / (float)screenWidth;
    if (key_state != KEY_CTRL)
      xy[1] = -(float)(ym - mouse_down_xy0[1]) / (float)screenHeight;
...
void MoveGenSlice(int xm, int ym) {
  ...
  case KEY_ALT: {
    ...
    gslice_xyz[2] =
        gslice_xyz0[2] -
        DENORMALIZE_Z(4 * (xyzbox - NORMALIZE_Z(gslice_xyz0[2])) * yy);
#ifdef pp_GLUI
    UpdateGsliceParms();
#endif
  } break;
...
#define ANGLE_FACTOR 0.25
  ...
  case KEY_SHIFT:
    ...
    yy = yy / (float)screenHeight;
    aperture_glui =
        CLAMP(aperture_glui0 + aperture_max * yy, aperture_min, aperture_max);
#define APERTURE 15
#define ZOOM 12
#ifdef pp_GLUI
...
void MouseDragCB(int xm, int ym) {
  ...
  if (colorbar_splitdrag == 1 && (showtime == 1 || showplot3d == 1) &&
      current_colorbar != NULL && current_colorbar->nsplits == 1) {
  ...
  if (rotation_type == ROTATION_3AXIS &&
      (key_state == KEY_NONE || key_state == KEY_SHIFT)) {
...
// #endif
//     if(scriptoutstream!=NULL&&key!='t'&&key!='r'&&key!='R'&&key!='
//     '&&key!='-'){
...
//         if(ndeviceinfo==0&&hrrptr!=NULL&&plot_option==DEVNO_HRRNO)plot_option_temp
//         = DEVNO_HRRYES;
//         if(ndeviceinfo>0&&hrrptr==NULL&&plot_option==DEVNO_HRRNO)plot_option_temp
//         = DEVYES_HRRNO; plot_option = plot_option_temp;
//       }
//       // 0 - device no, hrr no
...
//         printf("  use the W key to toggle clipping mode: disabled, clip
//         blockages and data, clip blockages, clip data\n"); printf("  use
//         x/y/z keys to activate/deactivate lower x/y/z clipping planes\n");
//         printf("  use X/Y/Z keys to activate/deactivate upper x/y/z clipping
//         planes\n"); printf("  use cursor and page down/up keys to move lower
//         clipping planes\n"); printf("  use SHIFT cursor and page down/up keys
//         to move upper clipping planes\n");
//       }
//       else{
...
//       }
//       if(blocklocation==BLOCKlocation_grid)printf("blocklocation: snapped to
//       grid\n"); if(blocklocation==BLOCKlocation_exact)printf("blocklocation:
//       as input\n");
...
//         if(texti->display==0){ // if any textures are hidden then show them
//         all
//           showhide_textures = 1;
//           break;
//         }
...
//         if(keystate==GLUT_ACTIVE_ALT&&strncmp((const char *)&key2, "r", 1) ==
//         0){
...
//         if(strncmp((const char *)&key2, "R",
//         1)==0&&keystate!=GLUT_ACTIVE_ALT){
...
//               plotn,visx_all,xp[meshi->plotx]);
//               fprintf(scriptoutstream,"SHOWPLOT3DDATA\n");
//               fprintf(scriptoutstream," %i %i %i %i %f\n",i+1,2,
//               plotn,visy_all,yp[meshi->ploty]);
//               fprintf(scriptoutstream,"SHOWPLOT3DDATA\n");
//               fprintf(scriptoutstream," %i %i %i %i %f\n",i+1,3,
//               plotn,visz_all,zp[meshi->plotz]);
//               fprintf(scriptoutstream,"SHOWPLOT3DDATA\n");
//               fprintf(scriptoutstream," %i %i %i %i %i\n",i+1,4,
//               plotn,visiso,plotiso[plotn-1]);
//             }
//             if(show_plot3dkeywords==1){
//               fprintf(scriptoutstream,"PLOT3DPROPS\n");
//               fprintf(scriptoutstream," %i %i 4
...
//             iplotx_all = GetGridIndex(clipinfo.xmin, XDIR, plotx_all,
//             nplotx_all); printf("on\n");
//           }
...
//             printf("on\n");
//             iplotx_all = GetGridIndex(clipinfo.xmax, XDIR, plotx_all,
//             nplotx_all);
//           }
...
//             iploty_all = GetGridIndex(clipinfo.ymin, YDIR, ploty_all,
//             nploty_all); printf("on\n");
//           }
...
//             iploty_all = GetGridIndex(clipinfo.ymax, YDIR, ploty_all,
//             nploty_all); printf("on\n");
//           }
...
//             iplotz_all = GetGridIndex(clipinfo.zmax, ZDIR, plotz_all,
//             nplotz_all); printf("on\n");
//           }
...
//             iplotz_all = GetGridIndex(clipinfo.zmin, ZDIR, plotz_all,
//             nplotz_all); printf("on\n");
//           }
...
//         if(npartthread_ids>1)printf("parallel particle loading: on(%i
//         threads)\n",npartthread_ids); if(npartthread_ids==1)printf("parallel
//         particle loading: on(1 thread)\n");
//       }
...
void KeyboardGlfw(GLFWwindow *window, int key, int scancode, int action,
                  int mods, int flag) {
  ...
  case GLFW_KEY_A:
    if (mods & GLFW_MOD_SHIFT) {
      ...
      if (rotation_type == EYE_CENTERED) {
        HandleMoveKeys(mods,256 + key2);
        break;
      }
      if ((visVector == 1 && nplot3dloaded > 0) || showvslice == 1 ||
          isZoneFireModel == 1) {
      } else {
        break;
      }
  ...
  case GLFW_KEY_Q:
    ...
    if (mods & GLFW_MOD_SHIFT) {
      ...
      for (i = 0; i < ntextureinfo; i++) {
        ...
        if (texti->loaded == 0 || texti->used == 0) continue;
        if (texti->display ==
            0) { // if any textures are hidden then show them all
          showhide_textures = 1;
          break;
        }
      }
      ...
      if (blocklocation == BLOCKlocation_grid)
        printf("blocklocation: snapped to grid\n");
      if (blocklocation == BLOCKlocation_exact)
        printf("blocklocation: as input\n");
      if (blocklocation == BLOCKlocation_cad) printf("blocklocation: cad\n");
      if (showedit_dialog == 1) {
        if (blocklocation == BLOCKlocation_exact) {
          blockage_as_input = 1;
    ...
    if (scriptoutstream != NULL) {
      if (nglobal_times > 0) {
        ...
        for (i = 0; i < nmeshes; i++) {
          ...
          fprintf(scriptoutstream, "SHOWPLOT3DDATA\n");
          fprintf(scriptoutstream, " %i %i %i %i %f\n", i + 1, 1, plotn,
                  visx_all, xp[meshi->plotx]);
          fprintf(scriptoutstream, "SHOWPLOT3DDATA\n");
          fprintf(scriptoutstream, " %i %i %i %i %f\n", i + 1, 2, plotn,
                  visy_all, yp[meshi->ploty]);
          fprintf(scriptoutstream, "SHOWPLOT3DDATA\n");
          fprintf(scriptoutstream, " %i %i %i %i %f\n", i + 1, 3, plotn,
                  visz_all, zp[meshi->plotz]);
          fprintf(scriptoutstream, "SHOWPLOT3DDATA\n");
          fprintf(scriptoutstream, " %i %i %i %i %i\n", i + 1, 4, plotn, visiso,
                  plotiso[plotn - 1]);
        }
      ...
      if (clip_rendered_scene != 0) {
        fprintf(scriptoutstream, "RENDERCLIP\n");
        fprintf(scriptoutstream, " %i %i %i %i %i\n", clip_rendered_scene,
                render_clip_left, render_clip_right, render_clip_bottom,
                render_clip_top);
      }
      if (vis_gslice_data == 1 || show_gslice_triangles == 1 ||
          show_gslice_triangulation == 1 || show_gslice_normal) {
        fprintf(scriptoutstream, "GSLICEVIEW\n");
        fprintf(scriptoutstream, " %i %i %i %i\n", vis_gslice_data,
                show_gslice_triangles, show_gslice_triangulation,
                show_gslice_normal);
        fprintf(scriptoutstream, "GSLICEPOS\n");
        fprintf(scriptoutstream, " %f %f %f\n", gslice_xyz[0], gslice_xyz[1],
                gslice_xyz[2]);
        fprintf(scriptoutstream, "GSLICEORIEN\n");
        fprintf(scriptoutstream, " %f %f\n", gslice_normal_azelev[0],
                gslice_normal_azelev[1]);
      }
      if (rflag == 0) {
        fprintf(scriptoutstream, "RENDERONCE\n");
      } else {
  ...
  case GLFW_KEY_S:
    if (mods & GLFW_MOD_SHIFT) {
      ...
      if (mods & GLFW_MOD_ALT) {
        ...
        if (rotation_type == EYE_CENTERED) {
          HandleMoveKeys(mods,GLFW_KEY_DOWN);
        } else {
          vectorskip++;
          if (vectorskip > 4) vectorskip = 1;
        }
  ...
  case GLFW_KEY_W:
    if (mods & GLFW_MOD_SHIFT) {
      ...
      // 'w'
      if (rotation_type == EYE_CENTERED) {
        HandleMoveKeys(mods,GLFW_KEY_UP);
      } else {
        vis_gslice_data = 1 - vis_gslice_data;
        // UpdateGsliceParms();
      }
  ...
  case GLFW_KEY_GRAVE_ACCENT:
    if (mods & GLFW_MOD_SHIFT) {
      ...
      if (slice_resolution_level > max_slice_resolution)
        slice_resolution_level = 0;
...
#define IMMERSED_SWITCH_CELLTYPE 0
  ...
  case '/':
    ...
    if (part_multithread == 1) {
      if (npartthread_ids > 1)
        printf("parallel particle loading: on(%i threads,streaks disabled)\n",
               npartthread_ids);
      if (npartthread_ids == 1)
        printf("parallel particle loading: on(1 thread, streaks disabled)\n");
    }
    if (part_multithread == 0) printf("parallel particle loading: off\n");
    // UpdateGluiPartFast();
    break;
  ...
  if (strncmp((const char *)&key2, "-", 1) != 0 &&
      ...
      strncmp((const char *)&key2, ",", 1) != 0 &&
      strncmp((const char *)&key2, ".", 1) != 0 &&
      strncmp((const char *)&key2, "_", 1) != 0 && (skip2 <= 0 || skip2 >= 10))
    return;

  if (clip_mode != CLIP_OFF && (strncmp((const char *)&key2, "<", 1) == 0 ||
                                strncmp((const char *)&key2, ",", 1) == 0 ||
                                strncmp((const char *)&key2, ">", 1) == 0 ||
                                strncmp((const char *)&key2, ".", 1) == 0)) {
    ...
    UpdateClipbounds(clipinfo.clip_xmin, &clip_i, clipinfo.clip_xmax, &clip_I,
                     current_mesh->ibar);
    UpdateClipbounds(clipinfo.clip_ymin, &clip_j, clipinfo.clip_ymax, &clip_J,
                     current_mesh->jbar);
    UpdateClipbounds(clipinfo.clip_zmin, &clip_k, clipinfo.clip_zmax, &clip_K,
                     current_mesh->kbar);
    return;
  }

  if (strncmp((const char *)&key2, "-", 1) == 0 ||
      strncmp((const char *)&key2, "_", 1) == 0) {
    FlowDir = -1;
  } else if (strncmp((const char *)&key2, " ", 1) == 0 ||
             strncmp((const char *)&key2, "=", 1) == 0 ||
             strncmp((const char *)&key2, "+", 1) == 0) {
...
void KeyboardCBGlfw(GLFWwindow *window, int key, int scancode, int action,
                    int mods) {
...
void HandleRotationType(int flag) {
  ...
  case ROTATION_3AXIS:
    ...
    if (showtrainer_dialog == 0 && flag == ROTATION_2AXIS &&
        rotation_type_old == EYE_CENTERED) {
    ...
    if (clip_mode != clip_mode_last) {
      if (startup == 1) {
        ...
        if (clip_mode == CLIP_BLOCKAGES_DATA)
          PRINTF("clipping blockages + data\n");
...
void SpecialKeyboardUpCB(int key, int x, int y) { resetclock = 1; }

/* ------------------ SpecialKeyboardCB ------------------------ */

void SpecialKeyboardCB(int mods,int key, int x, int y) {
...
#define P3_MODE 1
  ...
  switch (keymode) {
  case P3_MODE:
    HandlePLOT3DKeys(mods, key);
    stept = 0;
    break;
  case EYE_MODE:
    HandleMoveKeys(mods, key);
    break;
  default:
    ASSERT(FFALSE);
    break;
...
void HandlePLOT3DKeys(int mods, int key) {
  if (clip_commandline == 1) {
    switch (key) {
    case GLFW_KEY_LEFT:
    case GLFW_KEY_RIGHT:
...
void HandleMoveKeys(int mods, int key) {
  int state;
  float dx, dy;
...
#define LOOKANGLE_CHANGE 11.25
  ...
  case 256 + 'd':
    ...
    dy = INC_XY * (sin_azimuth);
    {
      ...
      if (mods & GLFW_MOD_SHIFT) local_speed_factor = 4.0;
...
void UpdateFrame(float thisinterval, int *changetime, int *redisplay) {
  ...
  if (showtime == 1 && ((stept == 1 && (float)thisinterval > frameinterval) ||
                        render_status == RENDER_ON ||
                        timebar_drag == 1)) { /* ready for a new frame */
    ...
    if (nglobal_times > 0) {
      *changetime = 1;
      if (stept == 1 && plotstate == DYNAMIC_PLOTS && timebar_drag == 0 &&
          render_status == RENDER_OFF) {
        /*  skip frames here if displaying in real time and frame rate is too
         * slow*/
        if (global_times != NULL && realtime_flag != 0 && FlowDir > 0) {
          ...
          if (nglobal_times > 1 &&
              ...
              (elapsed_time > global_times[nglobal_times - 1] ||
               elapsed_time < 0.0)) {
            elapsed_time = GMod(elapsed_time, global_times[nglobal_times - 1] -
                                                  global_times[0]) +
                           global_times[0];
          }
...
void AdjustY(cameradata *ca) {
  ...
  if (projection_type != PROJECTION_PERSPECTIVE ||
      update_saving_viewpoint > 0 || update_viewpoint_script > 0)
    return;
...
void ReshapeCB(GLFWwindow *window, int width, int height) {
...
void DoStereo(void) {
  if (stereotype == STEREO_TIME &&
      videoSTEREO == 1) { // temporal stereo (shuttered glasses)
...
void DoScriptLua(void) {
  ...
  if (runluascript == 1) {
    ...
    if (script_return_code != LUA_OK && script_return_code != LUA_YIELD &&
        exit_on_script_crash) {
      SMV_EXIT(1);
    }
  }
}
...
void DoScript(void) {
  ...
  if (nscriptinfo > 0 && current_script_command != NULL &&
    ...
    if (FILE_EXISTS(stop_filename) == YES) {
      fprintf(stderr, "*** Warning: stop file found.  Remove before running "
                      "smokeview script\n");
      SMV_EXIT(0);
    }
#endif
    ...
    if (current_script_command >= scriptinfo &&
        current_script_command->command == SCRIPT_SETVIEWPOINT &&
        update_viewpoint_script > 0) {
      advance_script = 0;
    } else {
      advance_script = 1;
...
#define NREPEATS 2
    ...
    if (advance_script == 1) { // don't advance command if Smokeview is
                               // executing a RENDERALL command
      if (nrenderonce == 0) {
  ...
  if (render_status == RENDER_OFF) {
    ...
    if (plotstate == DYNAMIC_PLOTS && nglobal_times > 0) {
      if (itimes >= 0 && itimes < nglobal_times &&
          ((render_frame[itimes] == 0 && stereotype == STEREO_NONE) ||
           (render_frame[itimes] < 2 && stereotype != STEREO_NONE))) {
        render_frame[itimes]++;
        stop_rendering = 0;
      }
    }
    if (render_mode == RENDER_NORMAL) {
      ...
      NewMemory((void **)&screenbuffers, resolution_multiplier *
      ...
      for (i = 0; i < resolution_multiplier; i++) {
        ...
        for (j = 0; j < resolution_multiplier; j++) {
          ShowScene(DRAWSCENE, VIEW_CENTER, 1, j * screenWidth,
                    i * screenHeight, NULL);
