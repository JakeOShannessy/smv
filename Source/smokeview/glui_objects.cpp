...
void AddCSVCurve(plot2ddata *plot2di, int index, int option){
  ...
  if(have_plot == 0 && nplots < PLOT2D_MAX_CURVES){
    ...
    if(strcmp(c_type, "devc")==0){
      curve->quantity = scase.devicecoll.deviceinfo[curve->csv_col_index-1].quantity;
    }
...
void GLUIGenPlotCB(int var){
  ...
  switch(var){
    ...
    case GENPLOT_SELECT_CURVE:
      ...
      if(BUTTON_plot_position != NULL){
        if(glui_plot2dinfo->curve_index<scase.devicecoll.ndeviceinfo){
        }
        else{
...
#define COLOR_EPS 5
    ...
    case GENPLOT_SET_PLOTPOS:
      if(glui_plot2dinfo->curve_index<scase.devicecoll.ndeviceinfo){
        ...
        plot_xyz = scase.devicecoll.deviceinfo[glui_plot2dinfo->curve_index].xyz;
    ...
    case GENPLOT_RESET_DEV_PLOTS:
      ...
      dev_pos = scase.devicecoll.deviceinfo[idevice_add].quantity;
      for(i = 0; i<nplot2dinfo; i++){
        plot2ddata *plot2di;
        curvedata *curvei;
    ...
    case GENPLOT_ADD_DEV_PLOTS:
      ...
      dev_quant = scase.devicecoll.deviceinfo[idevice_add].quantity;
      ...
      for(i=0;i<scase.devicecoll.ndeviceinfo;i++){
        ...
        icsv_cols = devi - scase.devicecoll.deviceinfo +1;
...
extern "C" void GLUIDeviceSetup(int main_window){
  ...
  if(scase.devicecoll.ndeviceinfo>0){
    ...
    if(GetNumActiveDevices()>0||sextras.isZoneFireModel==1){
      ...
      if(scase.devicecoll.nvdeviceinfo==0)ROLLOUT_velocityvectors->disable();
