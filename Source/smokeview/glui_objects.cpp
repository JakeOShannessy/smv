...
extern "C" void GLUIPlot2DSetup(int main_window){
  ...
  if((global_scase.csvcoll.ncsvfileinfo>0&&have_ext==0)||(global_scase.csvcoll.ncsvfileinfo>1&&have_ext==1)){
    ...
    ROLLOUT_plotproperties = glui_plot2d->add_rollout("plot properties(all plots)", true, PLOT2D_NEW_ROLLOUT);
   ...
   // BUTTON_plot_position = glui_plot2d->add_button_to_panel(PANEL_plot_position, "Set to device location", GENPLOT_SET_PLOTPOS, GLUIGenPlotCB);
    ...
    ROLLOUT_curve_properties = glui_plot2d->add_rollout("curve properties", false, PLOT2D_CURVE_ROLLOUT);
    ...
    ROLLOUT_plot_bounds = glui_plot2d->add_rollout("plot bounds(all plots)", false, PLOT2D_BOUNDS_ROLLOUT);
...
extern "C" void GLUIDeviceSetup(int main_window){
  ...
  if(global_scase.devicecoll.ndeviceinfo>0){
    ...
    ROLLOUT_smvobjects = glui_device->add_rollout_to_panel(PANEL_objects, "Objects", false, OBJECTS_ROLLOUT);
    ...
    if(GetNumActiveDevices()>0||global_scase.isZoneFireModel==1){
      ...
      ROLLOUT_windrose = glui_device->add_rollout_to_panel(PANEL_objects, "Wind rose", false, WINDROSE_ROLLOUT);
