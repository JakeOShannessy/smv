...
int SVimage2var(int rendertype, int woffset, int width, int hoffset, int height,
  ...
  glReadPixels(width_beg, height_beg, width2, height2, GL_RGB, GL_UNSIGNED_BYTE,
               OpenGLimage);
  ...
  *RENDERimage = gdImageCreateTrueColor(width2, height2);
  ...
  if(test_smokesensors == 1 && active_smokesensors == 1 &&
...
void DevicesHideAll() {
  for(size_t i = 0; i < scase.objectscoll.nobject_defs; i++) {
    sv_object *objecti = scase.objectscoll.object_defs[i];
    objecti->visible = 0;
  }
}
...
void Loadvolsmoke(int meshnumber) {
  ...
  else if(imesh >= 0 && imesh < scase.meshescoll.nmeshes) {
    meshdata *meshi;
    volrenderdata *vr;
...
void Loadvolsmokeframe(int meshnumber, int framenumber, int flag) {
  ...
  if(index > scase.meshescoll.nmeshes - 1) index = -1;
  for(size_t i = 0; i < scase.meshescoll.nmeshes; i++) {
    if(index == i || index < 0) {
      meshdata *meshi;
      volrenderdata *vr;
...
void ShowPlot3dData(int meshnumber, int plane_orientation, int display,
  ...
  if(meshnumber < 0 || meshnumber > scase.meshescoll.nmeshes - 1) return;

  meshi = scase.meshescoll.meshinfo + meshnumber;
...
int SetShowdevices(int ndevices_ini, const char *const *names) {
  ...
  for(size_t i = 0; i < ndevices_ini; i++) {
    ...
    obj_typei = GetSmvObject(&scase.objectscoll, tempname);
