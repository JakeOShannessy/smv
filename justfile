...
build:
    cmake -B cbuild -DLUA=ON -DGLUI=ON -DLUA_BUILD_BINARY=ON -DLUA_UTIL_LIBS=ON -DCMAKE_BUILD_TYPE=Debug
    cmake --build cbuild --config Debug -j6 -v
    cmake --install cbuild --config Debug --prefix dist-debug

# Build the release binaries
build-release:
    cmake -B cbuild -DLUA=ON -DGLUI=ON -DLUA_BUILD_BINARY=ON -DLUA_UTIL_LIBS=ON -DCMAKE_BUILD_TYPE=Release
