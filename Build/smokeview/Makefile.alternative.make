...
SMVLUACORE_FILES = smv.lua ssf.lua \
	ssfparser.lua ssfcommands.lua \
	clipping.lua bounds.lua \
	render.lua load.lua \
	view.lua tour.lua \
	iniparser.lua inioptions.lua \
	unload.lua constants.lua \
	camera.lua window.lua
...
ifeq ($(BUILD_TARGET),gnu_linux_64_db)
...
LIBS_PLAT = $(LIB_DIR_PLAT)/libglui.a \
	...
	$(LIB_DIR_PLAT)/libpng.a $(LIB_DIR_PLAT)/libz.a \
	$(LIB_DIR_PLAT)/libglut.a
FFLAGS    = -O0 -m64 -x f95-cpp-input -D pp_GCC -D pp_noappend -ffree-form -frecord-marker=4
CFLAGS    = -O0 -m64 -D pp_LINUX -Wno-write-strings
ifeq ($(LUA_SCRIPTING),true)
CFLAGS    += -D pp_LUA
SMVLUACORE_FILES += $(LIB_DIR_PLAT)/lpeg.so
LIBS_PLAT += $(LIB_DIR_PLAT)/liblua.a $(LIB_DIR_PLAT)/lpeg.so
...
CPP       = g++
FC        = gfortran
exe       = smokeview_linux_$(SMV_TESTSTRING)64
...
ifeq ($(BUILD_TARGET),mingw_win_64)

LIB_DIR_PLAT = $(LIB_DIR)/mingw_win_64
LIBS_PLAT = $(LIB_DIR_PLAT)/libglui.a \
	$(LIB_DIR_PLAT)/libgd.a $(LIB_DIR_PLAT)/libjpeg.a \
	$(LIB_DIR_PLAT)/libpng.a $(LIB_DIR_PLAT)/libz.a \
...
ifeq ($(LUA_SCRIPTING),true)
CFLAGS    += -D pp_LUA
LIBS_PLAT += $(LIB_DIR_PLAT)/lua53.dll
SMVLUACORE_FILES += $(LIB_DIR_PLAT)/lpeg.dll
LIBS_PLAT += $(LIB_DIR_PLAT)/lua53.dll $(LIB_DIR_PLAT)/lpeg.dll
endif
LFLAGS    = -m64 -static-libgcc -static-libstdc++ -lgcov
CC        = gcc
CPP       = g++
FC        = gfortran
exe       = smokeview_mingw_$(SMV_TESTSTRING)64.exe
execn     = $(bin)/$(exe)

excn = smokeview_mingw_64.exe
